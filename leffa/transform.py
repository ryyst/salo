...
def transform_movies(raw_data: Dict[str, Any], config: LeffaConfig) -> LeffaData:
    ...
    for movie_id, movie_info in movies_dict.items():
        ...
        sorted_shows = sorted(movie_info['shows'], 
                            key=lambda s: s.start_time if s.start_time else '')
    ...
    logger.info(f"Transformed {len(movies)} movies with total {sum(len(m.shows) for m in movies)} shows")
