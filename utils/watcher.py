...
class RunnerFileWatcher(FileSystemEventHandler):
    ...
    def __init__(self, debounce_delay: float = 2.0):
        ...
        # Map directories to runner names
        self.dir_to_runner = {
            "auki": "auki",
            "swimmi": "swimmi",
            "tori": "tori",
    ...
    def on_modified(self, event):
        ...
        runner_name = None
        for part in file_path.parts:
            if part in self.dir_to_runner:
                runner_name = self.dir_to_runner[part]
                break
    ...
    def schedule_runner(self, runner_name: str):
        ...
        self.debounce_timer = threading.Timer(
            self.debounce_delay, self.execute_pending_runners
        )
    ...
    def execute_pending_runners(self):
        ...
        for runner_name in sorted(runners_to_execute):
            ...
            if os.path.exists(config_file):
                ...
                Log.warning(
                    f"Config file not found for runner {runner_name}: {config_file}"
                )
